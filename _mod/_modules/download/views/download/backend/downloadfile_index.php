			...
			?>
				<tr id="row_<?php echo $row->id; ?>" class="<?php echo ($i % 2) ? 'even_row' : 'odd_row'; ?> <?php echo ($row->status != $statuses[0]) ? 'red_row' : ''; ?>">
					...
					<td>
						...
						<a class="download" title="Download" href="<?php echo URL::site(ADMIN.$class_name.'/download/'.base64_encode($upload_url.$row->file_name));?>">
					...
					<td align="center" style="margin:0 auto; vertical-align: middle">
						<div style="width: 34px;">
							...
							if ($row->order >= 1 && $row->order != $max_order->last_order($row->type_id)):
								echo '<a class="odr_up" title="Order Up" href="'.URL::site(ADMIN.$class_name).'/order/up/'.$row->id.'/'.$row->order.'/'.$row->type_id.'"></a>';
							endif;
							if ($row->order <= $max_order->last_order($row->type_id) && $row->order != $min_order->first_order($row->type_id) && $row->order != $max_order->last_order($row->type_id) + 1):
								echo '<a class="odr_down" title="Order Down" href="'.URL::site(ADMIN.$class_name).'/order/down/'.$row->id.'/'.$row->order.'/'.$row->type_id.'"></a>';
							endif;
							?>
						</div>
					</td>	

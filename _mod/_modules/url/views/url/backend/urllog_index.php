    ...
    <?php
        ...
        foreach ($listings as $row):
            ...
            <tr class="listing_<?php echo $class;?> <?php echo $class;?>" id="row_<?php echo $row->id;?>">
				...
				<?php 
					echo empty($row->click_time) ? '-' : $row->click_time;
				?>
				</td>
				<td class="bold">
					<?php echo Text::limit_words(strip_tags($row->shorturl),4);?>
				</td>					
				<td>
					<?php echo trim(Text::limit_chars(strip_tags($row->referrer),30)); ?>
				</td>
				<td align="left">
				<?php 
					echo empty($row->user_agent) ? '-' : $row->user_agent;
				?>
				</td>	
				<td align="left">
				<?php 
					echo empty($row->ip_address) ? '-' : $row->ip_address;
				?>
				</td>		                          
                <!--td width="11%">

...
class Controller_Backend_CareerDivision extends Controller_Backend_BaseAdmin {
    
    protected $careerDivision;
	...
	public function before () {
        ...
        $this->careerDivision	= new Model_CareerDivision;
		$this->file				= new Model_CareerDivisionFile;
    ...
    public function action_index() {       
		...
		//$this->careerDivision->find($where_cond);
		//exit();
		...
		if ($_POST) {
			...
			if ($this->id1 == 'select_action' && isset($_POST['check'])) {
				...
				foreach ($rows as $row) {
					$this->careerDivision->id	= $row;

					if (!$this->careerDivision->load())
						continue;

					$this->careerDivision->status	= $_POST['select_action'];
					$this->careerDivision->update();
				}
		...
		$total_rows		= $this->careerDivision->find_count($where_cond);
		$total_record 	= $total_rows;
		
		$listings	= $this->careerDivision->find($where_cond, $order_by, $per_page, $offset);
		...
		//$content			= array();
		$content			= View::factory('career/backend/'.$this->_class_name.'_index');
			...
			*/
			...
			if ($post->check()) {
				...
				$params	= array('subject'			=> strip_tags($fields['subject']),
								'name'				=> strtolower(str_replace(' ', '-', $fields['subject'])),
								'description'		=> $fields['description'],
								'status'			=> $fields['status']);

				$id		= $this->careerDivision->add($params);
	...
	public function action_view ( ) {
		$this->careerDivision->id	= $this->id1;
		...
		$where_cond			= array('division_id'	=> $this->careerDivision->id);
		...
		$content_vars		= array('career_division' => $this->careerDivision,
									'files'			=> $files,
	...
	public function action_edit ( ) {
		$division_id				= $this->id1;
		$this->careerDivision->id	= $division_id;
...
//			$post->add_callbacks('name', array($this, '_unique_name'));
			...
			if ($post->check()) {
				...
				$this->redirect(ADMIN . $this->_class_name.'/view/'.$this->careerDivision->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
			...
			$fields	= array(
							'name'				=> $this->careerDivision->name,
							'subject'			=> $this->careerDivision->subject,
							'description'		=> $this->careerDivision->description,
							'status'			=> $this->careerDivision->status);
	...
	public function action_delete ($id = '') {
		...
		$this->careerDivision->status	= 'deleted';
		if($this->careerDivision->update()) {
			echo 1;
		}	
		else {
			echo 0;
...
//		$this->redirect($redirect_url);
	...
	public function action_change() {
		...
		if ($this->request->post('check') !='') {
			...
			foreach ($rows as $row) {
				...
				$this->careerDivision->status	= $this->request->post('select_action');
				$this->careerDivision->update();
			}
    ...
    public function _unique_name (Validation $array, $field) {
		if (!isset($array[$field]))
			return;
		$name = $this->careerDivision->find_count(array('name' => $array[$field]));
		if ($name)

		...
		<tbody>
			...
			?>
				<tr id="row_<?php echo $row->id; ?>" class="<?php echo ($i % 2) ? 'even_row' : 'odd_row'; ?> <?php echo ($row->status != $statuses[0]) ? 'red_row' : ''; ?>">
					...
					<td><?php echo $i; ?></td>
					<td><?php echo str_repeat('&nbsp;', abs(1 * 5)); ?><strong><a href="<?php echo url::site(ADMIN.$class_name.'/view/' . $row->id); ?>"><?php echo text::limit_words(strip_tags($row->subject),6).' ('.text::limit_chars(strip_tags($row->name),25).')'; ?></a></strong></td>
					<td align="center">
					...
					<?php else: ?>
						<?php 
							echo isset($categories[$row->category_id]) ? 
							TEXT::limit_words(strip_tags($categories[$row->category_id]->subject, TRUE),2,'') : 
							'No Parent'; ?>
					<?php endif;?>
					</td>
					<td align="center"><?php echo ucfirst($row->description); ?></td>

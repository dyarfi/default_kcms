		...
		<tbody>
			...
			?>
				<tr id="row_<?php echo $row->id; ?>" class="<?php echo ($i % 2) ? 'even_row' : 'odd_row'; ?> <?php echo ($row->status != $statuses[0]) ? 'red_row' : ''; ?>">
					...
					<td><?php echo $i; ?></td>
					<td><strong><a href="<?php echo URL::site(ADMIN.$class_name.'/view/' . $row->id); ?>"><?php echo Text::limit_chars(strip_tags($row->subject),25); ?></a></strong></td>
					<td align="center"><?php echo Text::limit_words($row->description,10); ?></td>
					...
					<td>
						<a class="btn btn-mini functions" title="List" href="<?php echo URL::site(ADMIN.$class_name.'/index/' . $row->id); ?>"><div class="icon-list indentMin">List</div></a>

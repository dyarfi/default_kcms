...
class Controller_Backend_Portfolio extends Controller_Backend_BaseAdmin {
	...
	public function before () {
		...
		$this->category		= new Model_PortfolioCategory;
		$this->user			= new Model_User;

		$where_cond			= array('status'	=> 'publish');
		$this->portfolios		= $this->portfolio->find($where_cond);
		$this->categories	= $this->category->find($where_cond);

		$this->file			= new Model_PortfolioFile();
		$this->user			= new Model_User();
	...
	function action_index () {
		...
		$total_rows	= $this->portfolio->find_count($where_cond);
		$listings	= $this->portfolio->find($where_cond, $order_by, $per_page, $offset);
...
$category_id = $this->id1;
		...
		if ($_POST) {
			...
			if ($post->check()) {
				...
				$last_order	= $this->portfolio->find_count($where_cond) + 1;
				if (isset($fields['order']) && $fields['order'] < $last_order) {
					$where_cond	= array('category_id'	=> $fields['category_id'],
										'order >='		=> $fields['order']);
					$pages	= $this->portfolio->find($where_cond);
		...
		$order_by			= array('category_id'	=> 'ASC',
									'order'			=> 'ASC');
		$orders				= $this->portfolio->find($where_cond, $order_by);
	...
	public function action_view () {
		...
		if (!$this->portfolio->load()) {
			$this->redirect(ADMIN.$this->_class_name.'/error/invalid_request');
			return;
		}
		/** Views **/
		if ($this->portfolio->category_id != 0) {
			$category		= $this->category->load($this->portfolio->category_id);
			$category		= HTML::chars($category->title, TRUE);
		} else {
			$category		= 'This page doesn\'t have category';
		}
		if ($this->portfolio->order == 0) {
			$order			= '-';
		} else if ($this->portfolio->order != 1) {
			$where_cond		= array('category_id'	=> $this->portfolio->category_id,
									'order'			=> ($this->portfolio->order - 1));
			$pages		= $this->portfolio->find($where_cond, '', 1);
			if (isset($pages[0]))
		...
		$where_cond			= array('portfolio_id'	=> $this->portfolio->id);
		...
		Lib::_auto_image_manipulation($this->_upload_path, $this->file, $this->_prefs);
		/** Views **/
		$content_vars		= array('portfolio'		=> $this->portfolio,
									'category'		=> $category,
									'files'			=> $files,
									'order'			=> $order,
...
//			/print_r($fields['category_id']); exit(
			...
			if ($post->check()) {
				...
				$where_cond	= array('category_id'	=> $fields['category_id']);
				$last_order	= $this->portfolio->find_count($where_cond) + 1;
				if ($this->portfolio->category_id != $fields['category_id']) {
					$where_cond	= array('category_id'	=> $this->portfolio->category_id,
										'order >='		=> $this->portfolio->order);
					$orders		= $this->portfolio->find($where_cond);
					...
					} else if ($this->portfolio->order < @$fields['order']) {
						$where_cond	= array('category_id'	=> $fields['category_id'],
											'order >'		=> $this->portfolio->order,
											'order <='		=> $order,
											'status !='		=> 'deleted');
				...
				if ($category_id != '')
					$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->portfolio->id.'/'.$category_id);
				else
					$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->portfolio->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
				$errors 	= Arr::overwrite($errors, $post->errors('validation'));
			...
			$fields	= array('category_id'		=> $this->portfolio->category_id,
							'name'				=> $this->portfolio->name,
							'subject'			=> $this->portfolio->subject,
	...
	public function action_change() {
		...
		if ($this->request->post('check') !='') {
			...
			foreach ($rows as $row) {
				$this->portfolio->id	= $row;

				if (!$this->portfolio->load())
					continue;

				$this->portfolio->status	= $this->request->post('select_action');
				$this->portfolio->update();
			}
	...
	public function _unique_name (Validation $array, $field) {
		if (isset($this->portfolio->name) && $this->portfolio->name == $array[$field])
			return;

		$where_cond		= array('name'	=> $array[$field]);
		$name_exists	= ($this->portfolio->find_count($where_cond) != 0);

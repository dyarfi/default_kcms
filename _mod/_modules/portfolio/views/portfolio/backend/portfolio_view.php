...
<form class="form_details" action="<?php echo url::site(ADMIN.$class_name.'/edit/'.$portfolio->id); ?>" method="get" enctype="multipart/form-data">
		<div class="form_wrapper">
			...
			<?php if (isset($show_parent) && $show_parent) : ?>
			<div class="form_row">
				<label>Parent</label>
			...
			<?php if (isset($files[$row_name]) && count($files[$row_name]) != 0) : ?>	
				<fieldset>
					<legend><?php echo $row_params['label']; ?></legend>
					<?php if (!empty($files[$row_name])) : ?>
					<div class="form_row">
						<!--label><?php echo $row_params['label']; ?></label-->
						<div class="form_fields">
							<?php 
							if (is_file($upload_path.$files[$row_name]->file_name) && in_array($files[$row_name]->file_type, $readable_mime)) : ?>
							<div id="file_<?php echo $files[$row_name]->id; ?>">
								...
								<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="320" height="240" id="FLVPlayer">
									...
									<param name="scale" value="noscale" />
									<param name="salign" value="lt" />
									<param name="FlashVars" value="skinName=<?php echo URL::site(); ?>flash/Corona_Skin_2&amp;streamName=<?php echo URL::site(ADMIN.$class_name.'/download/'.$files[$row_name]->file_name); ?>&amp;autoPlay=false&amp;autoRewind=false" />
									<param name="swfversion" value="8,0,0,0" />
									<!-- This param tag prompts users with Flash Player 6.0 r65 and higher to download the latest version of Flash Player. Delete it if you don?t want users to see the prompt. -->
									<param name="expressinstall" value="Scripts/expressInstall.swf" />
									<!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->
		...
		<fieldset>
			...
			<?php foreach ($portfolio_files as $files): ?>
				<div class="pull-left img-thumbnail">
					<img class="img-rounded" src="<?php echo URL::site($portfoliofile_upload_url .'/thumbnail/'. $files->file_name);?>"/>
					<a href="<?php echo URL::site(ADMIN.'portfoliofile/delete/' . $files->id); ?>" title="Delete" class="label label-default delete_function">x</a>
				</div>
			<?php endforeach; ?>	
			</div> 
			<div class="clear topBotDiv20"></div>
			<input id="file_upload" class="file" type="file" name="files[]" data-url="<?php echo URL::site(ADMIN.'portfoliofile/upload/'.$portfolio->id)?>" multiple/>				
			<div class="clear"></div>				
		</fieldset>		
		<?php else : ?>		
		<fieldset>
			...
			<div class="clear topBotDiv20"></div>
			<input id="file_upload" class="input-sm" type="file" name="files[]" data-url="<?php echo URL::site(ADMIN.'portfoliofile/upload/'.$portfolio->id)?>" multiple/>
			<div class="clear"></div>			
		</fieldset>	
		<?php endif; ?>

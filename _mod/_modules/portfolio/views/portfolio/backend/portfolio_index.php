		...
		<tbody>
			...
			?>
				<tr id="row_<?php echo $row->id; ?>" class="<?php echo ($i % 2) ? 'even_row' : 'odd_row'; ?> <?php echo ($row->status != $statuses[0]) ? 'red_row' : ''; ?>">
					...
					<td><?php echo $i; ?></td>
					<td><?php echo str_repeat('&nbsp;', abs($row->sub_level * 5)); ?><strong><a href="<?php echo url::site(ADMIN.$class_name.'/view/' . $row->id); ?>"><?php echo text::limit_words(strip_tags($row->subject),6).' ('.text::limit_chars(strip_tags($row->name),25).')'; ?></a></strong>
						
					<?php if (in_array($files[$row->id]->file_type, $readable_mime)) : ?>
						<a href="#file_<?php echo $row->id; ?>" class="zoom">
							<img src="<?php echo ASSETS; ?>images/admin/<?php echo $mime_icon[$files[$row->id]->file_type]; ?>" alt="<?php echo $mime_icon[$files[$row->id]->file_type]; ?>" /></a>
						<a href="<?php echo URL::site(ADMIN.$class_name.'/download/'.$files[$row->id]->file_name); ?>">
						<img src="<?php echo ASSETS; ?>images/admin/disk.png" alt="<?php echo $files[$row->id]->file_name; ?>" /></a>
						<div id="file_<?php echo $row->id; ?>" class="">
							<?php if (substr($files[$row->id]->file_type, 0, strlen('image/')) == 'image/') : ?>
							<img width="400" src="<?php echo URL::site().$upload_url.$files[$row->id]->file_name; ?>" alt="<?php echo URL::site().$upload_url.$files[$row->id]->file_name; ?>" />
							<?php elseif (substr($files[$row->id]->file_type, 0, strlen('audio/')) == 'audio/') : ?>
							<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="320" height="20" id="FLVPlayer">
								...
								<param name="FlashVars" value="file=<?php echo URL::site(ADMIN.$class_name.'/download/'.$files[$row->id]->file_name); ?>&amp;backColor=c2c2c2&amp;frontColor=666666&amp;showDownload=false&amp;repeatPlay=false&songVolume=100" />
							...
							<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="320" height="240" id="FLVPlayer">
								...
								<param name="scale" value="noscale" />
								<param name="salign" value="lt" />
								<param name="FlashVars" value="skinName=<?php echo ASSETS; ?>flash/Corona_Skin_2&amp;streamName=<?php echo URL::site(ADMIN.$class_name.'/download/'.$files[$row->id]->file_name); ?>&amp;autoPlay=false&amp;autoRewind=false" />
								<param name="swfversion" value="8,0,0,0" />
								<!-- This param tag prompts users with Flash Player 6.0 r65 and higher to download the latest version of Flash Player. Delete it if you don�t want users to see the prompt. -->
								<param name="expressinstall" value="Scripts/expressInstall.swf" />
								<!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->

...
<?php if ($portfolio_id !='') : ?>
<h2><?php echo $module_menu; ?> on &quot;<?php echo HTML::chars($portfolios[$portfolio_id]->subject, TRUE); ?>&quot;</h2>
<?php else : ?>
				...
				<tr id="row_<?php echo $row->id; ?>" class="<?php echo ($i % 2) ? 'even_row' : 'odd_row'; ?> <?php echo ($row->status != $statuses[0]) ? 'red_row' : ''; ?>">
					...
					<td>
						...
						<div id="file_<?php echo $row->id; ?>" class="">
							...
							<img width="400" src="<?php echo URL::site().$upload_url.$row->file_name; ?>" alt="<?php echo URL::site().$upload_url.$row->file_name; ?>" />
					...
					<td><?php echo ($row->portfolio_id != 0 && isset($portfolios[$row->portfolio_id])) ? text::limit_words(HTML::chars($portfolios[$row->portfolio_id]->subject, TRUE),6) : 'No Album'; ?></td>

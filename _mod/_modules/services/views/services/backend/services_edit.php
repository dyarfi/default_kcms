...
<?php 
echo Form::open(URL::site(ADMIN.$class_name.'/edit/'.$services->id), array(
													'enctype' => 'multipart/form-data', 
													'method' => 'post', 
													'class' => 'general autovalid',
													'id' => 'edit-gallery'
												));	
echo Form::hidden('order', $services->order);
		...
		<?php echo sprintf($errors['order'], 'Order'); ?>
			<div class="form_row">
				...
				<select name="order" id="order">
					...
					<?php if (count($servicess) != 0) : ?>
					<optgroup label="After ...">
						<?php foreach ($orders as $row) : ?>
						<option value="<?php echo ($row->order + 1); ?>" <?php echo ($fields['order'] == ($row->order + 1)) ? 'selected="selected"' : ''; echo ($row->id == $services->id) ? ' disabled="disabled"' : ''; ?>><?php echo HTML::chars($row->title, TRUE); ?></option>
						<?php endforeach; ?>
					</optgroup>
					<?php endif; ?>
				</select>

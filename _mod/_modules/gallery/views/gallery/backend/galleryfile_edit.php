...
<div class="form_wrapper">	
	...
	<?php echo sprintf($errors['album_id'], 'Parent'); ?>
	<div class="form_row">
		<label>Album</label>
		<select name="album_id" id="album_id">
			<option value="">&nbsp;</option>
			<option value="0" <?php echo ($fields['album_id'] == 0) ? 'selected="selected"' : ''; ?>>No album</option>
			<?php foreach ($albums as $row) : ?>
			<option value="<?php echo $row->id; ?>" class="album_<?php echo $row->id; ?>" <?php echo ($fields['album_id'] == $row->id) ? 'selected="selected"' : ''; ?>><?php echo str_repeat('&nbsp;', ($row->sub_level + 1) * 5).text::limit_words(HTML::chars($row->subject, TRUE),8); ?></option>
			<?php endforeach; ?>
		</select>
	</div>

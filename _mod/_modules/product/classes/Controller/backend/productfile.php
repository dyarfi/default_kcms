...
class Controller_Backend_ProductFile extends Controller_Backend_BaseAdmin {
	...
	public function before () {
		...
		$this->_prefilter_keys	= array('product_id');
		...
		$this->_upload_path = (Lib::config($this->_class_name.'.productfile_upload_path') !== NULL) ? Lib::config($this->_class_name.'.productfile_upload_path') : array();
		$this->_upload_url = (Lib::config($this->_class_name.'.productfile_upload_url') !== NULL) ? Lib::config($this->_class_name.'.productfile_upload_url') : array();
		
		$this->file			= new Model_ProductFile;
		$this->product		= new Model_Product;
		
		$albums				= $this->product->find(array('status'=>'publish'));

		foreach ($albums as $row) {
			$buffers[$row->id]	= $row;
		}
		$this->products		= $buffers;
	...
	function action_index () {
		...
		if ($product_id != '')
			$this->_prefs	= array_merge($this->_prefs, (Lib::config('site.'.$this->_class_name.'_'.$product_id.'_index_fields') !== NULL) ? Lib::config('site.'.$this->_class_name.'_'.$product_id.'_index_fields') : array());
		...
		if ($product_id != '' && (($this->product = $this->product->load($product_id)) || ($this->product = Model_Product::instance()->load_by_name($product_id))))
			$product_id	= $this->product->id;
		else
			$product_id	= '';

		if ($product_id != '' && is_array($where_cond))
			$where_cond	= array_merge(array('product_id' => $product_id), $where_cond);
	...
	public function action_add () {
		...
		if ($product_id != '' && (($this->product = Model_Product::instance()->load($product_id)) || ($this->product = Model_Product::instance()->load_by_name($product_id))))
			$fields['product_id']	= $this->product->id;
		...
		if ($_POST) {
			...
			//$post->pre_filter('intval', 'product_id');
			...
			$post->rule('product_id', array($this, '_valid_product_id'), array(':validation', 'product_id'));
			...
			if ($post->check()) {
				...
				//if (!Upload::not_empty($post[$row_name]) || !Upload::type($post[$row_name],explode(',',$row_params['file_type'])) || !Upload::valid($post[$row_name]))
					...
					if ($file_name != '') {
						$params			= array('product_id'		=> $fields['product_id'],
												'name'			=> $fields['name'],
												'title'			=> $fields['title'],
	...
	public function action_view () {
		...
		if ($this->file->product_id != 0) {
			$album			= $this->product->load($this->file->product_id);
	...
	public function action_edit () {
		...
		if ($_POST) {
			...
			$post->rule('product_id', array($this, '_valid_product_id'));
			...
			$fields	= array('product_id'			=> $this->file->product_id,
							'name'				=> $this->file->name,
	...
	public function action_delete () {
		...
		if (!$this->file->load()) {
			$this->redirect('admin-cp/error/invalid_request');
			return;
		}
	...
	public function _valid_product_id (Validation $array, $field) {
		if ($array[$field] == 0)
			return TRUE;

		$where_cond		= array('id'	=> $array[$field]);
		$parent_exists	= ($this->product->find_count($where_cond) != 0);

		if (!$parent_exists)
			$array->error($field, 'invalid_product_id');
	}

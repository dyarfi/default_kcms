...
<?php 
echo Form::open(URL::site(ADMIN.$class_name.'/edit/'.$testimonial->id), array(
													'enctype' => 'multipart/form-data', 
													'method' => 'post', 
													'class' => 'general autovalid form_details',
													'id' => 'edit-gallery'
												));	
echo Form::hidden('order', $testimonial->order);
...
?>	
	<div class="form_wrapper">	
		...
		<div class="form_row">
			<label>Media Video <div class="red">* YouTube</div></label>
			<textarea name="media" id="media" class=""><?php echo $fields['media']; ?></textarea>
		</div>
			...
			<div class="form_row">
				...
				<select name="order" id="order">
					...
					<optgroup label="After ...">
						...
						<option value="<?php echo ($row->order + 1); ?>" <?php echo ($fields['order'] == ($row->order + 1)) ? 'selected="selected"' : ''; echo ($row->id == $testimonial->id) ? ' disabled="disabled"' : ''; ?>><?php echo HTML::chars($row->title, TRUE); ?></option>
						<?php endforeach; ?>
					</optgroup>
					<?php endif; ?>
				</select>

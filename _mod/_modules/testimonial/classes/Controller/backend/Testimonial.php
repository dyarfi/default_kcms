...
class Controller_Backend_Testimonial extends Controller_Backend_BaseAdmin {
	...
	public function action_add () {
		...
		if ($_POST) {
			...
			$post->rule('media', 'max_length', array(':value', 500));
	...
	public function action_view () {
		...
		Lib::_auto_image_manipulation($this->_upload_path, $this->file, $this->_prefs);
		/** Views **/
		$content_vars		= array('testimonial'		=> $this->testimonial,
									'files'			=> $files,
	...
	public function action_edit () {
		...
		if ($_POST) {
			...
			if ($post->check()) {
				...
				$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->testimonial->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
				$errors 	= Arr::overwrite($errors, $post->errors('validation'));
			...
			$fields	= array('name'				=> $this->testimonial->name,
							'subject'			=> $this->testimonial->subject,
	...
	public function _unique_name (Validation $array, $field) {
		if (isset($this->testimonial->name) && $this->testimonial->name == $array[$field])
			return;

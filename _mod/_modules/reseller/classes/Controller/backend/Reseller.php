...
class Controller_Backend_Reseller extends Controller_Backend_BaseAdmin {
	...
	public function action_add () {
		...
		$orders				= $this->reseller->find($where_cond, $order_by);
	...
	public function action_view () {
		...
		Lib::_auto_image_manipulation($this->_upload_path, $this->file, $this->_prefs);
		/** Views **/
		$content_vars		= array('reseller'		=> $this->reseller,
									'files'			=> $files,
	...
	public function action_edit () {
		...
		if ($_POST) {
			...
			if ($post->check()) {
				...
				$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->reseller->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
				$errors 	= Arr::overwrite($errors, $post->errors('validation'));
			...
			$fields	= array('name'				=> $this->reseller->name,
							'subject'			=> $this->reseller->subject,
	...
	public function _unique_name (Validation $array, $field) {
		if (isset($this->reseller->name) && $this->reseller->name == $array[$field])
			return;

		$where_cond		= array('name'	=> $array[$field]);
		$name_exists	= ($this->reseller->find_count($where_cond) != 0);

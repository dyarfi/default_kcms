...
<?php 
echo Form::open(URL::site(ADMIN.$class_name.'/edit/'.$banner->id), array(
													'enctype' => 'multipart/form-data', 
													'method' => 'post', 
													'class' => 'general autovalid form_details',
													'id' => 'edit-gallery'
												));	
echo Form::hidden('order', $banner->order);
			...
			<div class="form_row">
				...
				<select name="order" id="order">
					...
					<optgroup label="After ...">
						...
						<option value="<?php echo ($row->order + 1); ?>" <?php echo ($fields['order'] == ($row->order + 1)) ? 'selected="selected"' : ''; echo ($row->id == $banner->id) ? ' disabled="disabled"' : ''; ?>><?php echo HTML::chars($row->title, TRUE); ?></option>
						<?php endforeach; ?>
					</optgroup>
					<?php endif; ?>
				</select>

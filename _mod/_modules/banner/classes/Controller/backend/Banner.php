...
class Controller_Backend_Banner extends Controller_Backend_BaseAdmin {
	...
	public function action_add () {
		...
		$orders				= $this->banner->find($where_cond, $order_by);
	...
	public function action_view () {
		...
		Lib::_auto_image_manipulation($this->_upload_path, $this->file, $this->_prefs);
		/** Views **/
		$content_vars		= array('banner'		=> $this->banner,
									'files'			=> $files,
	...
	public function action_edit () {
		...
		if ($_POST) {
			...
			if ($post->check()) {
				...
				$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->banner->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
				$errors 	= Arr::overwrite($errors, $post->errors('validation'));
			...
			$fields	= array('name'				=> $this->banner->name,
							'subject'			=> $this->banner->subject,
	...
	public function _unique_name (Validation $array, $field) {
		if (isset($this->banner->name) && $this->banner->name == $array[$field])
			return;
